...
parser.add_argument('-Mw', dest='metatiles_width', type=int,
                    default=None, help='meta tiles width')
parser.add_argument('-Mh', dest='metatiles_height', type=int,
                    default=None, help='meta tiles height')
parser.add_argument('-m', dest='map', nargs='?',
                    choices=['!'], default='!', help='Include or exclude map data [exc]')
parser.add_argument('-mR', dest='map_reduction', nargs='*', choices=['t', 'p', 'f'], default=[
                    't', 'p', 'f'], help='Tile reduction: (t)iles, (p)al, (f)lipped [Rtpf]')
parser.add_argument('-mu', dest='map_dtype', choices=[
                    '8', '16', '32'], default='16', help='Map data type: u8, u16, u32 [u16]')
parser.add_argument('-gT', dest='transparent_color', default='FF00FF',
                    help='Transparent color; rrggbb hex or 16bit BGR hex [FF00FF]')
parser.print_help()
...
def getPalette(img):
    # transparent color
    tCol = [int(args.transparent_color[i:i+2], 16)
...
def getMetatiles(img, pal, metatilesW, metatilesH):
    tiles = getTiles(img, pal)
    print('tiles = %d x %d' % (len(tiles), len(tiles[0])))
    yTiles, xTiles = int(len(img)/8), int(len(img[0])/8)
    tileIDs = np.arange(xTiles*yTiles)
    print(tileIDs)
    tileIDs = np.reshape(tileIDs, (yTiles, xTiles))
    print(tileIDs)
    tileIDs = np.asarray(np.hsplit(tileIDs, xTiles / metatilesW)).flatten()
    print(tileIDs)

    print(len(tiles))
    flattened_tiles = [row for row in tiles for tile in row]
    metatiles = [flattened_tiles[i] for i in tileIDs]
    # tiles[tileIDs.flatten()] = tiles
    # tiles[]
    return metatiles


def hflip(tile):
    flipped = [tile[j] for i in range(0, len(tile), 8)
               for j in reversed(range(i, i+8))]
    return flipped


def vflip(tile):
    flipped = [tile[j]
               for i in reversed(range(0, len(tile), 8)) for j in range(i, i+8)]
    return flipped
...
def spritesToFile(filename, img, metatilesW, metatilesH):
    print('sprite(s):')
    ...
    with open(filename, 'w+') as fp:
        ...
        tiles = getTiles(img, pal)
        # tiles = getMetatiles(img, pal, metatilesW, metatilesH)
...
if(args.map != '!'):
    ...
    spritesToFile(os.path.splitext(args.imagefile)[0] + '.h',
                  img, args.metatiles_height, args.metatiles_width)
